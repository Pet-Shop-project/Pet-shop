...
router.post('/register', (req,res)=>
{
 ...
 console.log(password2)
      ...
      res.status(501).json({Msg: 'Passwords do not match'}) ;
      
    
    }
  
    else if (password.length < 6) {
      res.status(501).json({Msg: "Password must be at least 6 characters"});
        ...
        User.findOne({ email: email }).then(user => {
            if (user) {
              res.status(501).json({Msg: "Email already exists"});
                } else {
...
router.post('/login', (req, res, next) => {
  ...
  User.findOne({
    ...
    bcrypt.compare(password, user.password, (err, isMatch) => {
      if (isMatch) {
        let token = jwt.sign({username : user.name},'Secret',{expiresIn:'3h'})
